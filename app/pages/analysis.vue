...
<template>
  <div class="flex-1 bg-neutral-950 overflow-y-auto">
    <div class="max-w-[800px] mx-auto px-4 py-8 space-y-6">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold text-neutral-200">Analysis</h1>
        <p class="text-sm text-neutral-500">Statistical breakdown of your session performance.</p>
      </div>
      ...
      <template v-if="hasSession">
        ...
        <div class="rounded-xl bg-neutral-900/80 border border-neutral-800 p-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div>
            <p class="text-[10px] uppercase tracking-widest text-neutral-500">Bankroll</p>
            <p class="text-xl font-mono text-neutral-200">{{ hero ? formatCents(hero.bankroll) : '--' }}</p>
          </div>
          <div>
            <p class="text-[10px] uppercase tracking-widest text-neutral-500">Net P&amp;L</p>
            <p
          ...
          <div>
            <p class="text-[10px] uppercase tracking-widest text-neutral-500">Total Wagered</p>
            <p class="text-xl font-mono text-neutral-200">{{ formatCents(sessionStats.totalWagered) }}</p>
          </div>
          <div>
            <p class="text-[10px] uppercase tracking-widest text-neutral-500">Point Conv. Rate</p>
        ...
        <div class="rounded-xl bg-neutral-900/80 border border-neutral-800 overflow-hidden">
          <div class="px-5 py-3 border-b border-neutral-800">
            <h2 class="text-sm font-semibold text-neutral-300">Roll Distribution</h2>
          </div>
        ...
        <div class="rounded-xl bg-neutral-900/80 border border-neutral-800 overflow-hidden">
          <div class="px-5 py-3 border-b border-neutral-800">
            <h2 class="text-sm font-semibold text-neutral-300">Bet Performance</h2>
          </div>
          <div v-if="betTypeEntries.length === 0" class="px-5 py-8 text-center text-neutral-600 text-sm">
            No bets placed yet.
          </div>
      ...
      <div v-else class="rounded-xl bg-neutral-900/80 border border-neutral-800 px-5 py-12 text-center">
        <p class="text-neutral-500 text-sm">No active session. Start a game to see analysis here.</p>
      </div>
    </div>
  </div>
</template>
